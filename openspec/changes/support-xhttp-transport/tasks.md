...
- [ ] 1.1 从 `sing-box-throne` (`origin/xhttp`) 移植精简版 Xray buffer 工具集至 `libcore/protocol/vless/internal/`（包含 `xray/buf`, `xray/pipe`, `xray/bytespool`, `xray/signal`）
- [ ] 1.2 移植 XHTTP 核心传输驱动至 `libcore/protocol/vless/xhttp`（包含 `client.go`, `conn.go`, `dialer.go`, `http.go`, `mux.go`, `upload_queue.go`, `writer.go`），并定义 `XHTTPOptions`
- [ ] 1.3 编写静态依赖与符号审计脚本 `tools/diagnostics/audit_xhttp_libcore.py`，使用 `uv run` 执行，验证依赖与官方 sing-box v1.13.16 纯净兼容
