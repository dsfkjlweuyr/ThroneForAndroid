...
libcore 负责创建隔离测试 box、按受测 profile/outbound 拨号、服务发现、下载/上传或简单下载、采样和取消；Android 负责组队列、生命周期、进度对话框/通知和最终结果持久落地。JNI 边界使用 gomobile 可表达的简单请求参数、结果对象和会话句柄，避免直接暴露 Go channel 或复杂集合。

每个结果至少包含 profile 关联键、下载/上传速率与字节数、延迟、服务器名称/国家、错误和取消标志。Android 使用独立于 `ProxyEntity.ping/status` 的 profile 关联持久化字段或实体保存成功吞吐结果，避免把 bits/s 与毫秒单位混入同一字段；结果随节点列表重绑和页面重建重新读取。速度测试会话必须绑定 context cancel，并复用现有测试 box 的网络策略、接口 wrapper 与 fd protect 语义。
...
### 7. 最终吞吐结果持久显示在节点卡片测速信息区域

速度测试进度对话框和通知只承载运行时反馈，不是最终结果的唯一载体。每个节点成功完成后，Android 必须将该节点的最终吞吐结果原子写入专用持久化模型，并通知配置列表重绑对应节点；关闭进度界面、刷新列表或重新进入配置列表后仍从持久化模型恢复显示。

节点卡片沿用 URL RTT 所在的右下角测速信息区域，以主题 secondary text 对应的灰色辅助文字展示，不硬编码具体灰色值。下载+上传模式显示下载与上传速率，仅下载和简单下载模式只显示下载速率，仅上传模式只显示上传速率；格式必须明确上下行方向和速率单位。新成功结果原子覆盖该 profile 的旧速度结果；“清除测试结果”同时清除 URL RTT 状态和专用速度结果。失败或取消不得写入伪造的成功吞吐值；本批取消前已经成功完成的节点结果保持已保存状态，当前未完成节点和剩余队列不新增成功结果。
...
- [速度结果与现有 `ProxyEntity.ping/status` 模型不匹配] → RTT 继续使用现有字段；下载/上传结果采用 profile 关联的专用持久化字段或实体，不把吞吐量压入毫秒字段。数据库升级需提供安全默认值，旧用户升级后在首次测速前不显示虚构结果。
