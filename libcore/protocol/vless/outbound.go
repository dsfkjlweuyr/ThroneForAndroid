...
type V2RayTransportOptions struct {
	...
	XHTTPOptions xhttp.V2RayXHTTPOptions `json:"-"`
}
...
func (o *V2RayTransportOptions) UnmarshalJSON(bytes []byte) error {
	...
	o.Type = raw.Type
	if raw.Type == xhttp.V2RayTransportTypeXHTTP {
		return json.Unmarshal(bytes, &o.XHTTPOptions)
	}
	return json.Unmarshal(bytes, &o.V2RayTransportOptions)
}

func (o V2RayTransportOptions) MarshalJSON() ([]byte, error) {
	if o.Type == xhttp.V2RayTransportTypeXHTTP {
		data, err := json.Marshal(o.XHTTPOptions)
		if err != nil {
			return nil, err
		}
		...
		m["type"] = xhttp.V2RayTransportTypeXHTTP
		return json.Marshal(m)
	}
	return json.Marshal(o.V2RayTransportOptions)
}
...
func RegisterOutbound(registry *outbound.Registry) {
	outbound.Register[VLESSOutboundOptions](registry, C.TypeVLESS, NewOutbound)
}
...
func NewOutbound(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options VLESSOutboundOptions) (adapter.Outbound, error) {
	...
	if options.TLS != nil {
		outbound.tlsConfig, err = tls.NewClientWithOptions(tls.ClientOptions{
			...
			KTLSCompatible: (options.Transport == nil || options.Transport.Type == "") &&
	...
	if options.Transport != nil {
		if options.Transport.Type == xhttp.V2RayTransportTypeXHTTP {
			outbound.transport, err = xhttp.NewClient(ctx, outbound.dialer, outbound.serverAddr, options.Transport.XHTTPOptions, outbound.tlsConfig)
			if err != nil {
				return nil, E.Cause(err, "create client transport: xhttp")
			}
		} else {
			outbound.transport, err = v2ray.NewClientTransport(ctx, outbound.dialer, outbound.serverAddr, options.Transport.V2RayTransportOptions, outbound.tlsConfig)
			if err != nil {
